...
- 原因：判定に `class_exists('WPSecurityGuard')` を使っていた。PHPはファイル冒頭の（親を持たない）クラス宣言を**コンパイル時に束縛**するため、このファイル自身の `class WPSecurityGuard` が1行目を実行する前に定義済みになる。結果 `class_exists` は常に true を返し、**1つしか入っていないサイトでも必ず `return` して降りていた**。
- 症状：WPSG が読み込まれず、XML-RPC遮断・ユーザー名列挙対策・バージョン情報隠蔽・点検エンドポイント（`wpsg/v1`）がすべて無効になる。管理画面に「二重にインストールされています」の通知が出続ける。プラグイン一覧では有効に見えるため気づきにくい。
- 修正：実行時にしか定義されない定数で判定する（`defined('WPSG_MAIN_FILE')` → 直後に `define('WPSG_MAIN_FILE', __FILE__)`）。二重インストールの検出という目的は変わらない。
- 検証：正常系（1つだけ）で降りないこと、異常系（2つ目の読み込み）で致命的エラーにならず通知が1件出ること、クラスが利用可能なままであることを確認。2.9.2 の実装を同条件に置くと1つでも降りることも併せて確認した。
- **2.9.2 を導入済みのサイトは 2.9.3 へ更新が必要。** 2.9.2 のままだと無防備な状態が続く。
...
- 追加の理由：既存の `site_url` は `home_url()` の戻り値で、**`is_ssl()` によってスキームが上書きされる**。集中管理側は https で点検しているため、DBの保存値が `http://` のサイトでも `https://` として報告されていた。実際、保存値が http のまま運用されているサイトを 4 件（同一サーバー）取りこぼしていた。
- 外形からの判定には限界がある。nginx や .htaccess が WordPress に届く前に 301 で https へ転送している場合、WordPress の応答自体を観測できず保存値を推定できない（該当 23 件）。**保存値は WordPress の中でしか読めない**ため、点検項目として持つ必要がある。
- 既存の `site_url` は互換のため残す（表示URLとしては正しい）。判定には `home_option`／`siteurl_option` を使う。
- 読み取り専用の項目追加のみ。サイトの挙動・設定は一切変更しない。
- 点検モジュールのバージョンを 1.4.1 に。

## 2.9.0

- **SEO Guard（検索対策モジュール）を新設**。「屋号・法人名・代表者名だけで検索したときにシステムサイトが出てこない状態」を作るため、集中管理側（remakemanager）から各サイトの **noindex 付与・メタタグ変更**を一括で行えるようにした。各サイトを個別に開く運用を無くすのが目的。
- **書き込みの認証に Ed25519 の公開鍵署名を採用**（`WPSG_SEO_Signature`）。本プラグインは公開リポジトリで配布されており、同梱の既定トークン（`WPSG_Inspect_Token::DEFAULT_TOKEN`）はソースを読めば誰でも入手できるため、**書き込みを既定トークンには一切許さない**。集中管理側だけが秘密鍵を持ち、各サイトは公開鍵で検証する。ソースが公開されていても書き込みはできず、かつ**各サイトでの設定作業はゼロ**（自動更新で配布するだけ）。
- 署名対象に **`site_url` を含める**ため、他サイト宛の署名を使い回せない。`timestamp`（有効60秒・時計ズレ±300秒）＋`nonce`（transientで15分記録）でリプレイを拒否。ボディのSHA-256で改ざんを検知。
- **sodium が無い環境では書き込みを拒否**する（安全側）。可用性は `/seo/scan` が報告するので、展開前に対象サイトの充足を確認できる。
- 新エンドポイント（`wpsg/v1`）:
  - `GET /seo/scan` … 読み取り専用。対象4ページ（会社概要・特商法・プライバシー・利用規約）の検出と現在の noindex 状態、トップの title/description、テーマ情報、本文中の会社名・代表者名の出現箇所を返す。**DBを直接読むため外形チェックより正確**。トークン認証（既定トークン可）。
  - `GET /seo/batches` … 復元可能な適用バッチの一覧。
  - `POST /seo/apply` … 適用。**署名必須**。
  - `POST /seo/rollback` … バッチ単位で復元。**署名必須**。
- **安全装置**：`dry_run` 既定 ON（`false` の明示指定が無ければ絶対に書き込まない）／変更前の値の保存に失敗した操作は実行しない／操作は `page_noindex`・`meta_title`・`meta_desc` のホワイトリストのみ（任意のメタ更新は受け付けない）／対象は `page`・`post` のみ／1リクエスト50操作まで／監査ログを直近100件保存／`define('WPSG_DISABLE_SEO_WRITE', true)` で当該サイトの書き込みを完全遮断。
- ページ種別の判定はスラッグ＋タイトルの候補一致で行い、**判定できなかったページは返さない**（推測で確定させず、集中管理側で目視・手動割当する）。
- **フッター・著作権表記は「検出のみ」**（作業4）。修正は本文テキスト（作業3）と同じく手作業とし、自動置換は実装しない。指示書が「テンプレートファイルでの修正が効率的」としているため、修正計画がそのまま立てられるよう **6か所**を走査する：テンプレートファイル（**ファイル名＋行番号＋該当行**・子テーマと親テーマの両方・2階層まで・除外ディレクトリあり・400ファイル/300KB上限）／オプション／theme_mods／ウィジェット／**ナビメニュー項目**／サイト名(blogname)。
- ナビメニューとサイト名を対象に含めたのは実サイト調査の結果による。**テンプレートに著作権表記が無いのに `<footer>` 内へ会社名が出ているケース**（onlinesalon 系テーマ＝対象16件中11件）があり、フッターメニューの項目名やサイト名が発生源になりうるため。
- フッター該当は **会社名・代表者名を含む行を `high`、著作権表記だけの行を `low`** に分類し、high を先頭へ並べ替える（開発会社名の定型表記などノイズに埋もれないようにするため）。`high_hits` と `total_hits` を併記。
- SEOプラグイン非導入サイト向けに、WPSG 独自の noindex 一覧（option `wpsg_seo_noindex_ids`）と `wp_robots` フィルタを用意。メタタグ書き換えは現時点で **Yoast SEO のみ対応**（対象69件中67件がYoast）。
- 点検モジュールのバージョンを 1.4.0 に。
